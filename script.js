(() => {
  ...
  const cases = {
    approval: {
      ...
      summary:
        "A maintainable approval structure that preserves business controls while reducing unnecessary routing complexity.",
      challenge:
        "The approval logic had grown across many states, transitions, repeated actions, and exceptions. Processing was slower and support teams could not easily explain why a request followed a specific path.",
      approach:
        "Map the actual policy first, identify duplicated logic, separate validation from approval routing, consolidate states, and move unsuitable logic into a focused SuiteScript component only where it improved clarity.",
      result:
        "A clearer approval model with easier troubleshooting, more predictable routing, less maintenance risk, and better confidence during future changes.",
    },
    integration: {
      category: "INTEGRATION ARCHITECTURE",
      title: "Multi-system integration hub",
      summary:
        "Reliable synchronization between NetSuite and external platforms with visibility beyond the happy path.",
      challenge:
        "Records moved across systems, but failures were difficult to classify and ownership was unclear. A successful request did not always mean the business transaction was complete or reconciled.",
      approach:
        "Define source-of-truth rules, validation boundaries, idempotent behavior, retry strategy, structured error messages, monitoring views, and a reconciliation process for financially important records.",
      result:
        "More dependable data movement, faster exception handling, clearer support ownership, and a design that could be extended without creating hidden operational risk.",
    },
    revenue: {
      ...
      challenge:
        "The required financial detail was too large and complex for a simple synchronous Suitelet page. Users needed filtering, trustworthy totals, progress visibility, and exportable results.",
      approach:
        "Separate the user interface from Map/Reduce processing, store request parameters and run tokens, paginate results, expose job status, and generate controlled exports after processing completes.",
      result:
        "A scalable reporting workflow that gives finance users better self-service while protecting script governance, response time, and result accuracy.",
    },
    intercompany: {
      ...
      challenge:
        "Manual journals required careful subsidiary, account, entity, currency, and balancing decisions. Small mistakes created rework and slowed finance review.",
      approach:
        "Build a service layer for reusable lookups and validation, provide a preview of proposed lines, verify balancing and required entities, then create records only after explicit confirmation.",
      result:
        "A more controlled process with fewer preventable entry errors, clearer review, and reusable logic that can support future transaction automation.",
    },
    workorder: {
      ...
      challenge:
        "Operational users needed a simple external form while NetSuite required validated customer, job, parent work-order, part, and labor records in the correct sequence.",
      approach:
        "Expose customer and job reference data through a read RESTlet, validate submitted JSON through a separate create RESTlet, create the parent first, and then associate line records using the returned internal ID.",
      result:
        "Less duplicate entry, a clearer user experience, testable API contracts, and a foundation for future status updates or customer self-service.",
    },
    migration: {
      category: "DATA MIGRATION",
      title: "Controlled ERP data migration",
      summary:
        "Repeatable data movement with validation, reconciliation, and business sign-off built into the process.",
      challenge:
        "Source data contained inconsistent references, formatting differences, missing relationships, and balances that needed to reconcile after import.",
      approach:
        "Profile source data, define transformation rules, validate master references, separate errors from accepted rows, test in sandbox, reconcile totals, and preserve an auditable migration log.",
      result:
        "A safer migration process with clearer exceptions, repeatable test cycles, and stronger confidence that imported records matched the intended business data.",
    },
  };
  ...
  document.querySelectorAll("[data-case]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      Object.keys(caseNodes).forEach((key) => {
        caseNodes[key].textContent = caseStudy[key];
      });
      caseModal.showModal();
      document.body.classList.add("modal-open");
  ...
  document
    .querySelectorAll("[data-close-case]")
    .forEach((button) => button.addEventListener("click", () => caseModal?.close()));
  ...
  contactForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showToast(
        isTimeout
          ? "The form timed out. Please use the direct email link."
          : "Message not sent. Please use the direct email link.",
        true,
      );
